...
def Plugins(**kwargs):
    	return [PluginDescriptor(name=_("Multibox FHD Configtool"), description=_("Personalize your Multibox FHD (Skin by stein17 Mod RAED)"), where = PluginDescriptor.WHERE_MENU, fnc=menu)]

def menu(menuid, **kwargs):
	if config.skin.primary_skin.value == "Multibox/skin.xml":
		if menuid == "mainmenu":
			return [(_("Setup -") + " " + "Multibox FHD", main, "Multibox_Menu", 40)]
	return []
...
class Multibox_Config(Screen, ConfigListScreen):
	...
	def getInitConfig(self):
		...
		self.Multibox_color = NoSave(ConfigSelection(default=current, choices = choices))
		# sb
		current, choices = self.getSettings(self.default_sb_file, self.sb_file)
		self.Multibox_sb = NoSave(ConfigSelection(default=current, choices = choices))
		# clock
		current, choices = self.getSettings(self.default_clock_file, self.clock_file)
		self.Multibox_clock = NoSave(ConfigSelection(default=current, choices = choices))
		# infobar
		current, choices = self.getSettings(self.default_infobar_file, self.infobar_file)
		self.Multibox_infobar = NoSave(ConfigSelection(default=current, choices = choices))
		# background
		current, choices = self.getSettings(self.default_background_file, self.background_file)
		self.Multibox_background = NoSave(ConfigSelection(default=current, choices = choices))
		# sib
		current, choices = self.getSettings(self.default_sib_file, self.sib_file)
		self.Multibox_sib = NoSave(ConfigSelection(default=current, choices = choices))
		# ch_se
		current, choices = self.getSettings(self.default_ch_se_file, self.ch_se_file)
		self.Multibox_ch_se = NoSave(ConfigSelection(default=current, choices = choices))
		# ev
		current, choices = self.getSettings(self.default_ev_file, self.ev_file)
		self.Multibox_ev = NoSave(ConfigSelection(default=current, choices = choices))
		# emcsel
		current, choices = self.getSettings(self.default_emcsel_file, self.emcsel_file)
		self.Multibox_emcsel = NoSave(ConfigSelection(default=current, choices = choices))
		# movsel
		current, choices = self.getSettings(self.default_movsel_file, self.movsel_file)
		self.Multibox_movsel = NoSave(ConfigSelection(default=current, choices = choices))
		# ul
		current, choices = self.getSettings(self.default_ul_file, self.ul_file)
		self.Multibox_ul = NoSave(ConfigSelection(default=current, choices = choices))
		# myatile
		myatile_active = self.getmyAtileState()
		self.Multibox_active = NoSave(ConfigYesNo(default=myatile_active))
		self.Multibox_fake_entry = NoSave(ConfigNothing())
	...
	def getSettings(self, default_file, user_file):
		...
		return current[0], choices
            	...
            	if path.exists(self.skin_base_dir + "mySkin_off"):
                	if not path.exists(self.skin_base_dir + "Multibox_Selections"):
                    		chdir(self.skin_base_dir)
                    		try:
                        		rename("mySkin_off", "Multibox_Selections")
                    		except:
                        		pass

	def createConfigList(self):
		self.set_color = getConfigListEntry(_("Style:"), self.Multibox_color)
		self.set_sb = getConfigListEntry(_("ColorSelectedBackground:"), self.Multibox_sb)
		self.set_clock = getConfigListEntry(_("Clock:"), self.Multibox_clock)
		self.set_infobar = getConfigListEntry(_("Infobar:"), self.Multibox_infobar)
		self.set_background = getConfigListEntry(_("Background:"), self.Multibox_background)
		self.set_sib = getConfigListEntry(_("Secondinfobar:"), self.Multibox_sib)
		self.set_ch_se = getConfigListEntry(_("Channelselection:"), self.Multibox_ch_se)
		self.set_ev = getConfigListEntry(_("Eventview:"), self.Multibox_ev)
		self.set_emcsel = getConfigListEntry(_("EMC_Selection:"), self.Multibox_emcsel)
		self.set_movsel = getConfigListEntry(_("Movie_Selection:"), self.Multibox_movsel)
		self.set_ul = getConfigListEntry(_("Userlogo:"), self.Multibox_ul)
		self.set_myatile = getConfigListEntry(_("Enable %s pro:") % cur_skin, self.Multibox_active)
		...
		if len(self.Multibox_color.choices)>1:
			self.list.append(self.set_color)
		if len(self.Multibox_sb.choices)>1:
			self.list.append(self.set_sb)
		if len(self.Multibox_clock.choices)>1:
			self.list.append(self.set_clock)
		if len(self.Multibox_infobar.choices)>1:
			self.list.append(self.set_infobar)
		if len(self.Multibox_background.choices)>1:
			self.list.append(self.set_background)
		if len(self.Multibox_sib.choices)>1:
			self.list.append(self.set_sib)
		if len(self.Multibox_ch_se.choices)>1:
			self.list.append(self.set_ch_se)
		if len(self.Multibox_ev.choices)>1:
			self.list.append(self.set_ev)
		if len(self.Multibox_emcsel.choices)>1:
			self.list.append(self.set_emcsel)
		if len(self.Multibox_movsel.choices)>1:
			self.list.append(self.set_movsel)
		if len(self.Multibox_ul.choices)>1:
			self.list.append(self.set_ul)
		self.list.append(self.set_new_skin)
		self["config"].list = self.list
		self["config"].l.setList(self.list)
		if self.Multibox_active.value:
	...
	def changedEntry(self):
		if self["config"].getCurrent() == self.set_color:
			self.setPicture(self.Multibox_color.value)
		elif self["config"].getCurrent() == self.set_sb:
			self.setPicture(self.Multibox_sb.value)
		elif self["config"].getCurrent() == self.set_clock:
			self.setPicture(self.Multibox_clock.value)
		elif self["config"].getCurrent() == self.set_infobar:
			self.setPicture(self.Multibox_infobar.value)
		elif self["config"].getCurrent() == self.set_background:
			self.setPicture(self.Multibox_background.value)	
		elif self["config"].getCurrent() == self.set_sib:
			self.setPicture(self.Multibox_sib.value)
		elif self["config"].getCurrent() == self.set_ch_se:
			self.setPicture(self.Multibox_ch_se.value)
		elif self["config"].getCurrent() == self.set_ev:
			self.setPicture(self.Multibox_ev.value)
		elif self["config"].getCurrent() == self.set_emcsel:
			self.setPicture(self.Multibox_emcsel.value)
		elif self["config"].getCurrent() == self.set_movsel:
			self.setPicture(self.Multibox_movsel.value)
		elif self["config"].getCurrent() == self.set_ul:
			self.setPicture(self.Multibox_ul.value)
	...
	def keyGreen(self):
		if self["config"].isChanged():
			...
			# color
			self.makeSettings(self.Multibox_color, self.color_file)
			# sb
			self.makeSettings(self.Multibox_sb, self.sb_file)
			# clock
			self.makeSettings(self.Multibox_clock, self.clock_file)
			# infobar
			self.makeSettings(self.Multibox_infobar, self.infobar_file)
			# background
			self.makeSettings(self.Multibox_background, self.background_file)
			# sib
			self.makeSettings(self.Multibox_sib, self.sib_file)
			# ch_se
			self.makeSettings(self.Multibox_ch_se, self.ch_se_file)
			# ev
			self.makeSettings(self.Multibox_ev, self.ev_file)
			# emcsel
			self.makeSettings(self.Multibox_emcsel, self.emcsel_file)
			# movsel
			self.makeSettings(self.Multibox_movsel, self.movsel_file)
			# ul
			self.makeSettings(self.Multibox_ul, self.ul_file)
            		#Pro SCREENS
	...
	def restartGUIcb(self, answer):
		...
		global cur_skin
        	...
        	if path.exists(user_skin_file):
        	...
        	#if self.Multibox_active.value:
            	cprint("update_user_skin.self.Multibox_active.value")
            	user_skin = ""
            	...
            	if user_skin != '':
                	...
                	with open (user_skin_file, "w") as myFile:
                    		cprint("update_user_skin.self.Multibox_active.value write myFile")
