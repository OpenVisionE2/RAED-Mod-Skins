...
def trace_error():
    ...
    try:
        ...
        traceback.print_exc(file=open('/tmp/AtileHD.log', 'a'))
    except:
        pass
...
try:
        config.plugins.AtileHD.city = ConfigText(default="manama", visible_width = 250, fixed_size = False)       
        config.plugins.AtileHD.windtype = ConfigSelection(default="ms", choices = [
                ("ms", _("m/s")),
                ("fts", _("ft/s")),
                ("kmh", _("km/h")),
                ("mph", _("mp/h")),
                ("knots", _("knots"))])
        config.plugins.AtileHD.degreetype = ConfigSelection(default="C", choices = [
                ("C", _("Celsius")),
                ("F", _("Fahrenheit"))])
        config.plugins.AtileHD.weather_location= ConfigText(default="bh-BH", visible_width = 250, fixed_size = False)       
except:
        trace_error()
        pass
...
class WeatherLocationChoiceList(Screen):
	...
	def get_xmlfile(self,weather_city,weather_location):
                degreetype = config.plugins.AtileHD.degreetype.value
...
"""
	...
	self.list.append(getConfigListEntry(_("---MSN Weather---"), self.myAtileHD_fake_entry))
	self.list.append(getConfigListEntry(_("Refresh interval in minutes:"), config.plugins.AtileHD.refreshInterval))
        self.list.append(getConfigListEntry(_("Temperature unit:"), config.plugins.AtileHD.degreetype))
        self.find_city = getConfigListEntry(_("Location #press OK to change:"), config.plugins.AtileHD.city)
    ...
    def keyOk(self):
        ...
        if sel is not None and sel == self.set_new_skin:
            ...
            countriesFile='/usr/lib/enigma2/python/Plugins/Extensions/AtileHD/countries'
    ...
    def choicesback(self, select):
        if select:
            ...
            config.plugins.AtileHD.weather_location.value=self.country.lower()+"-"+self.country.upper()
            config.plugins.AtileHD.weather_location.save()
    ...
    def citiesback(self,select):
        if select:
            ...
            config.plugins.AtileHD.city.setValue(weather_city)
            self.createConfigList()
    ...
    def keyGreen(self):
        if self["config"].isChanged() or self.updateEntries == True:
            ...
            config.plugins.AtileHD.city.value = config.plugins.AtileHD.city.value
            config.plugins.AtileHD.degreetype.value = config.plugins.AtileHD.degreetype.value
            for x in self["config"].list:
                x[1].save()
            configfile.save()
...
class AtileHD_About(Screen):
    skin = """
  <screen name="AtileHD_About" position="center,center" size="700,250" title="AtileHD info">
    <eLabel text="(c)2014/2015 by j00zek Mod BY RAED" position="0,15" size="698,50" font="Regular;28" halign="center"/>
    <eLabel text="Based on AtileHD skin by schomi / plugin by VTi" position="5,69" size="686,57" font="Regular;18" halign="center"/>
    <eLabel text="http://forum.xunil.pl" position="5,129" size="672,50" font="Regular;24" halign="center"/>
